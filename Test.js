...
const front = new THREE.TextureLoader().load("./models/skybox/front.jpg");
const back = new THREE.TextureLoader().load("./models/skybox/back.jpg");
const right = new THREE.TextureLoader().load("./models/skybox/right.jpg");
const left = new THREE.TextureLoader().load("./models/skybox/left.jpg");
const top= new THREE.TextureLoader().load("./models/skybox/top.jpg");
const bottom = new THREE.TextureLoader().load("./models/skybox/bottom.jpg");
...
function init() {
	...
	for (let i = 0; i < 6; i++)
		materialArray[i].side = THREE.DoubleSide;//je sais que je peux faire juste backside aussi
...
function addSphere(px, py, vx, vy, color, isStriped, number) {
	...
	let skyTexture = textureLoader.load('models/sky_reflection.png');
	...
	if (number <= 5) {
		shaderLoader.load('glsl/glass.fs.glsl', function(data) {
			sphereMaterial.fragmentShader = data;
			sphereMaterial.needsUpdate = true;
		});

		shaderLoader.load('glsl/glass.vs.glsl', function(data) {
